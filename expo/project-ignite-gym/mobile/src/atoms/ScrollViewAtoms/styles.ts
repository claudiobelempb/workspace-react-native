...
export const StyledContainer = styled.ScrollView<Props>`
  ${({ theme, ...props }) => css`
    width: 100%;
    position: relative;
    ${props.$bg?.$background &&
    ...
    min-height: ${props.$height?.$minHeight && `${props.$height.$minHeight}px`};
    max-height: ${props.$height?.$maxHeight && `${props.$height.$maxHeight}px`};
